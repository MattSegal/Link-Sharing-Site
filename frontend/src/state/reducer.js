const reducers = {
  ...
  ERROR_SEARCH: (state, action) =>
    mapSearch(state, action, search => ({
      items: [],
      updating: false,
    })),
  REQUEST_LINKS: (state, action) =>
  ...
  ERROR_LINK_LIST: (state, action) =>
    mapLinks(state, action, links => ({
      // Set link collection to 'not-updating' state
      ...links,
      updating: false,
    })),
  RECEIVE_ADD_LINK: (state, action) =>
    mapLinks(state, action, links => ({
      // Set link collection to 'not-updating' state
      ...links,
  ...
  REQUEST_LINK: (state, action) =>
    mapLinks(state, action, links => ({
      ...
      items: links.items.map(link =>
        link.id === action.link.id ? { ...link, updating: true } : { ...link }
      ),
    })),
  RECEIVE_EDIT_LINK: (state, action) =>
    mapLinks(state, action, links => ({
  ...
  ERROR_LINK_SPECIFIC: (state, action) =>
    mapLinks(state, action, links => ({
      // Set specific link to 'not-updating'
      ...links,
      items: links.items.map(link =>
        link.id === action.link.id ? { ...link, updating: false } : { ...link }
      ),
    })),
  RECEIVE_DELETE_LINK: (state, action) =>
    mapLinks(state, action, links => ({
      // Remove deleted link
      ...links,
  ...
  REQUEST_BOOKMARK_LINK: (state, action) => ({
    ...
    loggedInUser: {
      ...state.loggedInUser,
      bookmarks: setLinkUpdating(
        state.loggedInUser.bookmarks,
        action.link.id,
        true
      ),
    },
...
const setLinkUpdating = (links, link_id, is_updating) =>
  links.map(link =>
